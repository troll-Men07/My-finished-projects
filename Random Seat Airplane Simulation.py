...
# ---------- Choose boarding style ----------
print("\nBoarding options:")
print(" 1. One-by-one boarding  → Passengers enter individually.")
print(" 2. Group boarding       → Several passengers enter at once (like 5).")
mode = input("Choose boarding type (1 or 2, default 2): ").strip()
if mode not in ["1", "2"]:
    print("⚙️ Invalid or empty choice, using default: Group boarding (2).")
    mode = "2"
...
while not all_seats_full(plane):
    ...
    # --- Boarding mode selection ---
    if mode == "1":  # One-by-one boarding
        new_group = []
        if next_passenger_id <= total_passengers:
            new_group.append({'id': next_passenger_id, 'row': 0})
            next_passenger_id += 1
            time_elapsed += 0.5  # individual enters slower
            actions.append(f"Passenger {new_group[0]['id']} entered (+0.5s).")
    else:  # Group boarding
        new_group = []
